...
// Horus' `/InsAltCliente` rejects INS_ESTADUAL/INS_MUNICIPAL outright
// ("Não foi reconhecido o parâmetro"), even with a valid value — so the
// customer (and thus the order) never syncs if inscription is filled in.
// The store enters inscrição estadual/municipal manually inside Horus,
// so this field is intentionally never sent.
const buildCustomerBody = (customer, appData, customerHorus, storeId) => {
  const body = {
...
const exportCustomerToHorus = async ({ appSdk, storeId, auth }, customerId, opts = {}) => {
  ...
  if (isCreate && documentNumber && documentType) {
    ...
    const body = buildCustomerBody(customer, appData, customerHorus, storeId)
...
module.exports = exportCustomerToHorus
module.exports.buildCustomerBody = buildCustomerBody
